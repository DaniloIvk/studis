...
async function adminStats() {
	const [studentCount, professorCount, courseCount, gradeAgg, passedCount, totalApplications, grades] =
		await Promise.all([
			...
			database.examApplication.count({ where: { status: { in: ['PASSED', 'FAILED'] } } }),
			database.grade.findMany({ select: { value: true } }),
		]);

	const passRate = totalApplications > 0 ? Math.round((passedCount / totalApplications) * 100) : 0;

	const distribution = [6, 7, 8, 9, 10].map((grade) =>
		grades.filter((g) => Math.round(g.value) === grade).length,
	);
	...
	return {
		...
		stats: {
			...
			averageGrade: gradeAgg._avg.value ? Number(gradeAgg._avg.value.toFixed(2)) : 0,
...
async function professorStats(professorId: number) {
	const [myCourseCount, studentCount, grades, upcomingExamCount] = await Promise.all([
		database.course.count({ where: { professorId } }),
		database.enrollments.count({
	...
	const avgGrade =
		grades.length > 0
			? Number((grades.reduce((sum, g) => sum + g.value, 0) / grades.length).toFixed(2))
			: 0;
...
class DashboardController extends Controller {
	public async index({ response }: ControllerContract) {
		...
		switch (user.role) {
			case 'ADMIN':
				return response.json({ data: await adminStats() });
			case 'PROFESSOR':
				return response.json({ data: await professorStats(user.id) });
			case 'STUDENT':
				return response.json({ data: await studentStats(user.id) });
			default:
